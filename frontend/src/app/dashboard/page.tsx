...
export default function DashboardPage() {
  ...
  const loadAllTransactions = useCallback(async () => {
    try {
      ...
      console.error("Erreur récupération toutes transactions", err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadTransactions();
    loadAllTransactions();
  }, [loadInitialBalance, loadTransactions, loadAllTransactions]);

  const currentBalance = React.useMemo(() => {
    const totalTx = allTransactions.reduce((sum, tx) => sum + tx.amount, 0);
    return initialBalance + totalTx;
  }, [allTransactions, initialBalance]);
